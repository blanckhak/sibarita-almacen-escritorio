...
router.post('/', verificarToken, soloRoles('admin'), async (req, res) => {
  const { nombre, categoria, unidad_medida_id, codigo_interno } = req.body
  if (!nombre || !nombre.trim()) {
    return res.status(400).json({ error: 'El nombre del producto es requerido' })
  }
  try {
    const result = await pool.query(
      `INSERT INTO productos (nombre, categoria, unidad_medida_id, codigo_interno)
       VALUES ($1, $2, $3, $4) RETURNING id`,
      [nombre.trim(), categoria?.trim() || null, unidad_medida_id || null, codigo_interno?.trim() || null]
    )
...
router.put('/:id', verificarToken, soloRoles('admin'), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE productos SET nombre=$1, categoria=$2, unidad_medida_id=$3, codigo_interno=$4
       WHERE id=$5 RETURNING id`,
      [nombre.trim(), categoria?.trim() || null, unidad_medida_id || null, codigo_interno?.trim() || null, req.params.id]
    )
